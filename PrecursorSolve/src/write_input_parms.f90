...
!---
    ...
    write(outfile_unit, fmt='(a,es23.16)'), 'Taking constant time steps of: ', delta_t

    write(outfile_unit, fmt='(a,es23.16)'), 'Integrating in time up to: ', tmax

    write(outfile_unit, fmt='(a,es23.16)'), 'Starting calculation at time: ', t_initial

    write(outfile_unit, fmt='(a,I6)'), 'Total number of elements in the &
                                             system: ', num_elem
    ...
    write(outfile_unit, fmt='(a,es23.16)'), 'Total initial starting power: ', total_power_initial

    write(outfile_unit, fmt='(a,I6)'), 'Maximum number of nonlinear iterations', max_nl_iter

    write(outfile_unit, fmt='(a,es23.16)'), 'Size of each element [cm]: ', elem_size
    ...
    write(outfile_unit, fmt='(a,es23.16)'), 'Average nuetron generation time: ', gen_time

    write(outfile_unit, fmt='(a,es23.16)'), 'Reactivity input: ', reactivity_input

    write(outfile_unit, fmt='(a,es23.16)'), 'Save the spatial solution vectors at &
